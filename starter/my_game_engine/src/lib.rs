pub mod ffi {
    ...
    pub fn rust_write_text(text: &str, y: f32) {
        let num_chars = text.chars().count() as i32;
        let mut x = 400.0 - (num_chars as f32 * 20.0) / 2.0;
        //rust_clear_screen();
        for c in text.chars() {
            rust_write_char(c, x, y);
            x += 20.0;
        }
        //rust_update_game_window();
    }

    pub fn rust_write_char(c: char, x: f32, y: f32) {
        let mut coords = vec![];
        match c {
            'A' => {
                coords.extend(
            ...
            '!' => {
                coords.extend([(5.0, -15.0), (5.0, -10.0), (5.0, 0.0)].iter());
            }
            '0' => {
                coords.extend(
                    [
        ...
        for &(x_off, y_off) in &coords {
            spawn_sprite!(sprite, x + x_off, y + y_off, 4, 5, 255, 255, 255);
        }
    }
}
